const backend = {
cities: [
      {
        "id": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "name": "Gotham",
        "votingPopulation": 1859005,
        "absence": 185900,
        "presence": 1673105
      },
      {
        "id": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "name": "Asgard",
        "votingPopulation": 1850577,
        "absence": 111034,
        "presence": 1739543
      },
      {
        "id": "a7abb068-5ba0-4ec0-80e5-415219416c4a",
        "name": "Themyscira",
        "votingPopulation": 1625877,
        "absence": 146328,
        "presence": 1479549
      },
      {
        "id": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "name": "Metropolis",
        "votingPopulation": 1107096,
        "absence": 99638,
        "presence": 1007458
      },
      {
        "id": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "name": "Smallville",
        "votingPopulation": 1648006,
        "absence": 98880,
        "presence": 1549126
      }
    ],
    candidates: [
      {
        "id": "15744592-dd25-49bf-b117-f9c6098a4cf0",
        "name": "IronMan",
        "username": "ironman"
      },
      {
        "id": "1692e359-02a1-4a93-ac73-abca70251935",
        "name": "BlackWidow",
        "username": "blackwidow"
      },
      {
        "id": "187b1c0c-887f-41e2-a5de-00231191a468",
        "name": "Superman",
        "username": "superman"
      },
      {
        "id": "1f7954dc-c31c-4f29-8046-bb86ea96fb31",
        "name": "CaptainMarvel",
        "username": "captainmarvel"
      },
      {
        "id": "2c7ff125-0518-4397-b0de-727d47914131",
        "name": "Thor",
        "username": "thor"
      },
      {
        "id": "2fe23a92-9fd2-4b58-9c3c-dcc230a77106",
        "name": "GreenLantern",
        "username": "greenlantern"
      },
      {
        "id": "32cce90d-b2ef-47da-9ec5-13b5078c94a3",
        "name": "SpiderMan",
        "username": "spiderman"
      },
      {
        "id": "3465d8bd-b643-46e2-9781-56246040b1f3",
        "name": "CaptainAmerica",
        "username": "captainamerica"
      },
      {
        "id": "4fc98a0b-4a1d-426b-8324-25c665a56111",
        "name": "Aquaman",
        "username": "aquaman"
      },
      {
        "id": "763e3a75-8e48-4298-a7c4-5a8a59ed70dc",
        "name": "Batman",
        "username": "batman"
      },
      {
        "id": "80c5de49-94c3-48ff-b08e-c1a02199aaeb",
        "name": "WonderWoman",
        "username": "wonderwoman"
      },
      {
        "id": "87eed362-e4a8-4e67-9bfd-1f4cd35491a6",
        "name": "Antman",
        "username": "antman"
      },
      {
        "id": "bded7e2b-a580-4e3d-b587-9e83fa2b5097",
        "name": "Flash",
        "username": "flash"
      }
    ],

election: [
      {
        "id": "09194903-aafb-402e-8e5f-305bece66fbb",
        "cityId": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "candidateId": "32cce90d-b2ef-47da-9ec5-13b5078c94a3",
        "votes": 136142
      },
      {
        "id": "0d5eca36-1696-42f4-adf1-840c10028abd",
        "cityId": "a7abb068-5ba0-4ec0-80e5-415219416c4a",
        "candidateId": "763e3a75-8e48-4298-a7c4-5a8a59ed70dc",
        "votes": 177545
      },
      {
        "id": "1090e9a2-16a3-432a-ad3f-b4d6e1ff1dd0",
        "cityId": "a7abb068-5ba0-4ec0-80e5-415219416c4a",
        "candidateId": "2c7ff125-0518-4397-b0de-727d47914131",
        "votes": 96283
      },
      {
        "id": "128dfcc8-7b3e-42c1-bd50-dee63df4f7d2",
        "cityId": "a7abb068-5ba0-4ec0-80e5-415219416c4a",
        "candidateId": "80c5de49-94c3-48ff-b08e-c1a02199aaeb",
        "votes": 866550
      },
      {
        "id": "17ba6ddc-2c31-4423-ab5c-d1f0d92489ee",
        "cityId": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "candidateId": "15744592-dd25-49bf-b117-f9c6098a4cf0",
        "votes": 223190
      },
      {
        "id": "2572e7f0-0865-47b0-98cc-2ad426d8bd51",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "187b1c0c-887f-41e2-a5de-00231191a468",
        "votes": 369682
      },
      {
        "id": "2ad2d44a-cbf0-4a4d-8816-07723b20bddd",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "4fc98a0b-4a1d-426b-8324-25c665a56111",
        "votes": 128954
      },
      {
        "id": "2f3aad80-997e-4a26-b35f-cd0537822b58",
        "cityId": "a7abb068-5ba0-4ec0-80e5-415219416c4a",
        "candidateId": "bded7e2b-a580-4e3d-b587-9e83fa2b5097",
        "votes": 195300
      },
      {
        "id": "3ba6db6b-d02a-4bc7-b9da-f32a3152180c",
        "cityId": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "candidateId": "2fe23a92-9fd2-4b58-9c3c-dcc230a77106",
        "votes": 168076
      },
      {
        "id": "4bc6efe5-a902-47cf-82bd-d1f78543956e",
        "cityId": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "candidateId": "32cce90d-b2ef-47da-9ec5-13b5078c94a3",
        "votes": 98203
      },
      {
        "id": "526668ee-dc5f-427a-b82e-b24946cff3e5",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "2fe23a92-9fd2-4b58-9c3c-dcc230a77106",
        "votes": 120542
      },
      {
        "id": "64c3b0c4-c51d-4061-af59-18b3cc65482f",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "4fc98a0b-4a1d-426b-8324-25c665a56111",
        "votes": 160618
      },
      {
        "id": "689c6891-1ac3-424b-ae9c-78c827164004",
        "cityId": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "candidateId": "2c7ff125-0518-4397-b0de-727d47914131",
        "votes": 580396
      },
      {
        "id": "6a6fa27e-41e2-47f7-921f-bf1188b8e9c9",
        "cityId": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "candidateId": "3465d8bd-b643-46e2-9781-56246040b1f3",
        "votes": 226482
      },
      {
        "id": "6b2f55d2-1fcf-410e-9829-86349c989212",
        "cityId": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "candidateId": "763e3a75-8e48-4298-a7c4-5a8a59ed70dc",
        "votes": 330513
      },
      {
        "id": "7270081c-59de-445d-a34d-0ffc729b3a8d",
        "cityId": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "candidateId": "1f7954dc-c31c-4f29-8046-bb86ea96fb31",
        "votes": 221153
      },
      {
        "id": "83fc6900-59cc-4223-b9f8-a34aab2ff15d",
        "cityId": "a7abb068-5ba0-4ec0-80e5-415219416c4a",
        "candidateId": "32cce90d-b2ef-47da-9ec5-13b5078c94a3",
        "votes": 143871
      },
      {
        "id": "8d420a6e-5485-4628-9254-699f5d015e5d",
        "cityId": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "candidateId": "2fe23a92-9fd2-4b58-9c3c-dcc230a77106",
        "votes": 152175
      },
      {
        "id": "93d61be3-4f4d-4800-9418-0f80c6155762",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "2fe23a92-9fd2-4b58-9c3c-dcc230a77106",
        "votes": 45691
      },
      {
        "id": "a5327230-899d-48b5-831a-5d53c9b926f7",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "87eed362-e4a8-4e67-9bfd-1f4cd35491a6",
        "votes": 201491
      },
      {
        "id": "b22869ba-0d07-4297-98be-f649a67b3ee5",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "763e3a75-8e48-4298-a7c4-5a8a59ed70dc",
        "votes": 81241
      },
      {
        "id": "b89ae529-9638-4eb2-9510-4749b146fee1",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "187b1c0c-887f-41e2-a5de-00231191a468",
        "votes": 118476
      },
      {
        "id": "bab20ec3-5595-49eb-8915-b952ed581a37",
        "cityId": "d2dab6a2-3029-45a5-89f2-fcbaee387758",
        "candidateId": "4fc98a0b-4a1d-426b-8324-25c665a56111",
        "votes": 216877
      },
      {
        "id": "bb1759c8-455e-4ead-a066-c7939e7cbe20",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "2c7ff125-0518-4397-b0de-727d47914131",
        "votes": 622003
      },
      {
        "id": "c1e6e57e-6dd5-4793-8237-8e4ee882f428",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "1f7954dc-c31c-4f29-8046-bb86ea96fb31",
        "votes": 164901
      },
      {
        "id": "c5fef307-c78b-4cd5-8782-5278d874628d",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "1692e359-02a1-4a93-ac73-abca70251935",
        "votes": 101281
      },
      {
        "id": "d2d8d0ac-9b69-4be1-9355-c74d276726bb",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "87eed362-e4a8-4e67-9bfd-1f4cd35491a6",
        "votes": 334621
      },
      {
        "id": "e24be34c-922c-4932-9211-8c5ce882c0af",
        "cityId": "680df3f5-c5bf-48ae-bda0-5b667fd89197",
        "candidateId": "bded7e2b-a580-4e3d-b587-9e83fa2b5097",
        "votes": 151944
      },
      {
        "id": "e25f0f56-3dec-4111-af6c-e463f4de985c",
        "cityId": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "candidateId": "bded7e2b-a580-4e3d-b587-9e83fa2b5097",
        "votes": 140903
      },
      {
        "id": "e2a1f0d1-fa44-438b-ba5a-be28eb206c63",
        "cityId": "a27c86ce-d99a-4f4f-8cbb-37302754734e",
        "candidateId": "2c7ff125-0518-4397-b0de-727d47914131",
        "votes": 794559
      },
      {
        "id": "f9273068-33a4-456b-967f-84f3ce0abbcf",
        "cityId": "bfb650fc-ff1c-41ce-92ba-372c43cb21e5",
        "candidateId": "bded7e2b-a580-4e3d-b587-9e83fa2b5097",
        "votes": 79118
      }
    ]
  }


  function apiGetElections ( ){
    const { cities, candidates, election  } = backend
    const results = election.sort((a,b) => {
      if(a.votes > b.votes) return -1
      if(a.votes < b.votes) return 1
      return 0
     }).map((e) =>{
      return {...e, city: cities.filter(city =>city.id === e.cityId),
        cadidate: candidates.filter(candidate => candidate.id === e.candidateId)
      }
    })


    results.sort((a,b) => {return a.votes > b.votes})
    console.log(results)
    return results

  }

  function sortNameCity(){
    const { cities } = backend
    const cidadesSort = cities.sort(( a,b )=> {return a.name.localeCompare(b.name)})
    //console.log('cidades ordenadas', cidadesSort)
    return cidadesSort 
  }

  export { backend, apiGetElections, sortNameCity }